        ...
        <S.PostListWrapper>
          ...
          <S.PostList>
            {Array.isArray(data) && data.map((data, index) => (
              <Link key={data.id} to={`/Board/notices/${data.id}`} onClick={() => handleViewCount(data.id)}>
                <S.PostItem>
                  <S.PostNumber>{index + 1 + currentPage * pageSize}</S.PostNumber>
                  <S.PostTitleContainer>{data.title.length > 13 ? `${data.title.substring(0, 13)}...` : data.title}</S.PostTitleContainer>
                  <S.PostDate>{formatDate(data.createdAt)}</S.PostDate>
                  <S.PostAuthor>{data.createdBy}</S.PostAuthor>
                  <S.ViewCount> {data.view}</S.ViewCount>
                </S.PostItem>
              </Link>
            ))}
          </S.PostList>
